...
function retornaPaises()
{
    ...
    $index = 0;
    while ($row = mysqli_fetch_array($resultado)) { // loop to store the data in an associative array.
         $retorno[$index] = $row;
         $index++;
    }
...
function retornaTodosPosts()
{
    ...
    $sql = " SELECT DATE_FORMAT(t.data_inclusao, '%d %b %Y') AS data_inclusao,t.id_usuario as p_usuario, t.post,t.img_post,t.id_post, u.nome,u.sobrenome,t.titulo,i.img FROM post AS t JOIN perfil AS u ON (t.id_usuario = u.id_usuario) JOIN img_perfil as i on (u.id_usuario = i.id_usuario) ORDER BY t.data_inclusao DESC ";
