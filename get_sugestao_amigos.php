...
$sql = " Select * from (Select * from usuarios where id NOT IN (select id_usuario From amizade where id_usuario = $id_usuario )) teste JOIN img_perfil AS i ON (id = i.id_usuario) JOIN perfil AS p on (p.id_usuario = i.id_usuario) where id IN (Select id from usuarios where id IN (Select id_amigo from convite where id_usuario = $id_usuario )) ";
...
$sql = " SELECT * FROM usuarios AS u JOIN img_perfil AS i ON (u.id = i.id_usuario)  JOIN perfil AS p ON (p.id_usuario = u.id) where id not  IN (SELECT id_amigo FROM amizade where id_usuario = $id_usuario UNION SELECT id_amigo FROM convite WHERE id_usuario = $id_usuario) AND id <> $id_usuario";
